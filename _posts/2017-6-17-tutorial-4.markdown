...
The $$ \sigma^2 $$ on the right hand side of the first equation is the variance of the noise (from $$ \epsilon $$). As real-world data must have some noise, for all intents and purposes this is a constant (hence the name "irreducible error").
...
Finally, the third term, $$ \sigma^2 $$ is the _irreducible error_. It’s irreducible because no matter what model we use, the error will always be there. This is because the $$ \sigma^2 $$ term comes directly from the noise in our training data: $$ \epsilon $$. For all intents and purposes it is a constant. Note that because both the bias term and the variance term are non-negative (the bias because it’s being squared, and the variance by definition), $$ \sigma^2 $$ is a lower bound for the mean squared error. 
...
Which are exactly the frist and second terms. As for the third term, we can directly calculate the expectation using linearity of expectation:
...
(The last equality holds because $$ f $$ is a deterministic function. It's average at any point is simply going to be it's value at that point). Thus:
...
Again, we have $$ y = f + \epsilon $$. So, from the definition of variance:

\begin{align}
\operatorname{Var}[y] &= \operatorname{E}[y^2] - \operatorname{E}[y]^2 \\\
&= \operatorname{E}[f^2 + 2f\epsilon + \epsilon^2] - \operatorname{E}[f + \epsilon]^2 \\\
&= \operatorname{E}[f^2] + 2\operatorname{E}[f]\operatorname{E}[\epsilon] + \operatorname{E}[\epsilon^2] - \operatorname{E}[f]^2 \\\
&= \operatorname{E}[\epsilon^2]
\end{align}

Where we used linearity of expectation, the fact that $$ f $$ and $$ \epsilon $$ are independent, and $$ \operatorname{E}[\epsilon] = 0 $$.

Again, using the definition of variance, we have:

\begin{align}
\operatorname{Var}[\epsilon] &= \operatorname{E}[\epsilon^2] - \operatorname{E}[\epsilon]^2 \\\
&= \operatorname{E}[\epsilon^2]
\end{align}

But $$ \operatorname{Var}[\epsilon] = \sigma^2 $$! And substituting, we obtain our decomposition:
