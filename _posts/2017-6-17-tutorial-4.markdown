...
In the field of machine learning this incredibly important problem is known as the bias-variance dilemma. It’s entirely possible to have state-of-the-art algorithms, the fastest computers, and the most recent GPUs but if your model overfits or underfits to the training data, its predictive powers are going to be complete garbage.
...
<div style="margin: 0px 15%;">
<p style="font-size: 16px;">
Details: We are training a locally-weighted linear regression model (LOWESS) on a sine wave with added gaussian noise. (Thank you to <a target="_blank" href="https://gist.github.com/agramfort/850437">agramfort</a> for the LWLR code). The smoothing parameter is used as a proxy for complexity, and the error is the average L1 distance.
</p>
</div>
...
The bias-variance tradeoff results from using our necessarily flawed data to reconstruct this perfect function $$ f(x) $$. We want to ignore the noise term $$ \epsilon $$, but the noise is mashed up with the real data $$ f(x) $$, so if we ignore the noise too much, we end up ignoring the real data as well.
...
The bias-variance tradeoff results from using our necessarily flawed data to reconstruct this perfect function $$ f(x) $$. We want to ignore the noise term $$ \epsilon $$ by increasing our bias, but the noise is mashed up with the real data $$ f(x) $$, so if we ignore the noise too much, we end up ignoring the real data too, which leads to underfitting.
...
Let’s break this down a bit. We’re showing that the mean squared error (remember from [tutorial 1]({{ site.baseurl }}/2016/11/06/tutorial-1/)?) $$ \operatorname{E} \left[ (y - \hat f(x))^2 \right] $$ can be broken down into a “bias” term, a “variance” term, and a “sigma” term (the $$ \operatorname{E} $$ thing means “expectation of” and is just a fancy way of saying “average”). That is, how “wrong” our model, $$ \hat f(x) $$, is at predicting $$ y $$ can be broken down into those three components.
